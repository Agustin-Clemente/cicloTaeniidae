document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadParasites() {
        try {
            const response = await fetch('parasitos.json');
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            parasiteMapping = await response.json();
        } catch (error) {
            console.error('No se pudo cargar el archivo de parásitos:', error);
        }
    }
    ...
    const checkCycle = () => {
        if (Object.keys(parasiteMapping).length === 0) {
            console.error('Los datos de parásitos aún no están cargados.');
            return;
        }

        const currentImageIds = {
        hospedadorDefinitivo: getImageId(document.getElementById('person-img').src),
        parasitoAdulto: getImageId(document.getElementById('desarrolla').src),
        hospedadorIntermediario: getImageId(document.getElementById('animals-img').src),
        formaJuvenil: getImageId(document.getElementById('forma-juvenil').src),
        ambiente: getImageId(document.getElementById('feces-img').src),
        formaInfestante: getImageId(document.getElementById('forma-infestante').src)
    };

    for (const parasite in parasiteMapping) {
        if (parasiteMapping.hasOwnProperty(parasite)) {
            const correctMapping = parasiteMapping[parasite];
            let isCorrect = true;

            for (const key in correctMapping) {
                if (correctMapping.hasOwnProperty(key)) {
                    if (currentImageIds[key] !== correctMapping[key]) {
                        isCorrect = false;
                        break;
                    }
                }
            ...
            if (isCorrect) {
                Swal.fire({
                    title: '¡Ciclo Correcto!',
                    html: `¡Felicidades! Has armado el ciclo de <span style="color: #FFD700; font-weight: bold;">${parasite}</span>.`,
                    icon: 'success',
                    color: "#716add",
                    background: "black",
        ...
        Swal.fire({
            title: 'Ciclo Incorrecto',
            text: 'El ciclo que armaste no es correcto. ¡Inténtalo de nuevo!',
            icon: 'error',
            color: "#716add",
            background: "black",
            customClass: { popup: 'my-swal-border' }
    ...
    document.getElementById('check-button').addEventListener('click', checkCycle);
...
window.onload = function () {
    Swal.fire({
        text: "¡Arrastra y suelta las imágenes para formar tu ciclo!",
        color: "#716add",
        background: "black",
        customClass: {
